...
class TriviaTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.database_path = "postgres://{}/{}".format('suliman@localhost:5432', self.database_name)
    ...
    def test_search_with_results(self):
        res = self.client().post('/questions/searchresult', json={'searchTerm': 'flask'})
        data = json.loads(res.data)
    ...
    def test_search_without_results(self):
        res = self.client().post('/questions/searchresult', json={'searchTerm': 'sulimanaltalebUdacityFSND'})
        data = json.loads(res.data)
    ...
    def test_search_error(self):
        ...
        res = self.client().post('/questions/searchresults', json={'searchTerm': 'flask',})
        data = json.loads(res.data)
    ...
    def test_delete_question(self):
        newQuestion = Question(question='is this going to be deleted?', answer='Yes, of course',difficulty=5 , category=1)
        newQuestion.insert()
        res = self.client().delete("/questions/{}".format(newQuestion.id))
        data = json.loads(res.data)

        confirmed_deletion = Question.query.filter(Question.id == newQuestion.id).one_or_none()
